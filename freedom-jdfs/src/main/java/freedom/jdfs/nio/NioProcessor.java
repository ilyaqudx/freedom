...
public class NioProcessor {
	 ...
	 * 完成操作的任务队列
	 * */
	private ConcurrentLinkedQueue<StorageTask> completeTaskQueue = new ConcurrentLinkedQueue<StorageTask>();
	...
	final class Processor implements Runnable {
		...
		private void storage_recv_notify_read(SelectionKey key) {
			...
			storageTask.length = (int) Math.min(storageTask.size,
					clientInfo.remaining());
			clientSocketRead(session, storageTask);
		}

		private int storage_send_add_event(StorageTask storageTask) {
			// storageTask.offset = 0;//TODO Notice
			// 不能在这儿设置为0,暂时为了测试下载注释掉,上传这儿取消了肯定有问题.
		...
		private void client_sock_write(short event, StorageTask storageTask) {
			 ...
			 * LogKit.error(String
			 * .format("client ip : %s,recv error event %d,close connection"
			 * ,storageTask.clientIp,event),NioProcessor.class); //TODO
			 * task_finish_clean_up(storageTask); return; }
			 */
			...
			while (true) {
				 ...
				 */
				try {
					bytes = storageTask.session.getChannel().write(
							storageTask.data.buf());
				} catch (IOException e) {
					e.printStackTrace();
					LogKit.error(String.format(
							"client ip : %s,write failed,error info",
							storageTask.clientIp, e.getMessage()), this
							.getClass());
				...
				if (bytes < 0) {
					 ...
					 * set_send_event(pTask); } else if (errno == EINTR) {
					 * continue; } else {
					 * logError("file: "__FILE__", line: %d, " \
					 * "client ip: %s, recv failed, " \
					 * "errno: %d, error info: %s", \ __LINE__, pTask.client_ip,
					...
					LogKit.error("send failed, set write event.",
							NioProcessor.class);
					// TODO task_finish_clean_up(pTask);
				...
				if (storageTask.offset >= storageTask.length) {
					/*
					 * TODO if (set_recv_event(storageTask) != 0) { return; }
					 */

					clientInfo.totalOffset += storageTask.length;
					if (clientInfo.totalOffset >= clientInfo.totalLength) {
						/*
						 * TODO if (clientInfo.totalLength ==
						 * ProtoCommon.HEADER_LENGTH && ((TrackerHeader
						 * *)storageTask.data).status == EINVAL) {
						 * logDebug("file: "__FILE__", line: %d, "\
						 * "close conn: #%d, client ip: %s", \ __LINE__,
						 * storageTask.event.fd, storageTask.client_ip);
						 * task_finish_clean_up(storageTask); return; }
						 */
					...
					} else // continue to send file content
					{
						...
						storageTask.offset = 0;
						storageTask.length = (int) Math.min(storageTask.size,
								clientInfo.fileContext.end
										- clientInfo.fileContext.offset);
						/* continue read from file */
						StorageServer.context.storageDioService
								.storage_dio_queue_push(storageTask);
					}
		 ...
		 * read client data
		 * 
		 * 数据读完了,也删除了当前的读事件.为什么还有读事件不停的来.查看一下是否读取的数据为0,读完后, 自己的BYTEBUFFER没有可读的空间
		 * pos = limit
		 * **/
		private void clientSocketRead(NioSession session,StorageTask storageTask) {
			StorageClientInfo clientInfo = storageTask.clientInfo;
			if (clientInfo.canceled) {
				return;
			}
			...
			// TODO Notice
			/*
			 * fast_timer_modify(&pTask.thread_data.timer, &pTask.event.timer,
			 * g_current_time + g_fdfs_network_timeout);
			 */
			while (storageTask.data.hasRemaining()) {
				try {
					...
					ProtocolCodec.decode(session,storageTask, clientInfo);
				} catch (IOException e) {
					try {
